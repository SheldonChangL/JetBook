 ...
 * 聚合 I-06（#82）寫入的 `ai.query` 稽核事件（metadata 帶 input/outputTokens）。
 * 需精準控制 createdAt 以驗曆日切桶／時間窗，故直接插入 audit_logs（recordAiUsage 只用 now()）。
  ...
  it("按曆日聚合次數與 tokens；排除非 ai 事件與逾期事件；缺 outputTokens 以 0 計", async () => {
    ...
    await db.insert(auditLogs).values([
      // 今日：兩筆 ai.query → 次數 +2、tokens +160（150 + 10）
      {
        ...
        targetType: "ai",
        metadata: { model: "llm", inputTokens: 100, outputTokens: 50, latencyMs: 200, mode: "chat" },
        createdAt: now,
      },
      {
        actorId: actor.id,
        ...
        metadata: { model: "bge-m3", inputTokens: 10, latencyMs: 20, mode: "semantic" },
        createdAt: now,
      },
      // 昨日：一筆 ai.query → 次數 +1、tokens +10
      {
        ...
        action: AI_QUERY_AUDIT_ACTION,
        targetType: "ai",
        metadata: { model: "llm", inputTokens: 5, outputTokens: 5, latencyMs: 100, mode: "chat" },
      ...
      // 逾 30 日（時間窗外，應排除）
      {
        ...
        action: AI_QUERY_AUDIT_ACTION,
        targetType: "ai",
        metadata: { model: "llm", inputTokens: 1000, outputTokens: 1000, latencyMs: 300, mode: "chat" },
